...
import Model_Event, { initialEvent } from '../../../../models/events'
import { createEvent, fetchEvent } from '@/api/admin/event'
...
export default function Accounts() {
    ...
    const [formInput, setFormInput] = React.useState<Model_Event>(initialEvent)
    const breadcrumbs = [
    ...
    const fetchData = async () => {
        ...
        try {
            const Event: [] = await fetchUser(['admin']); 
            console.log(Event)
            setUserlist(Event); // Assuming `users` is an array of objects with the 'PlayerName' property
        } catch (error) {
    ...
    const formSubmit = async (event: React.ChangeEventHandler<HTMLInputElement> | React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        if (formInput.eventName != "" &&
            formInput.eventDetails  != "" &&
            formInput.plasadaPercent != "" &&
            formInput.status != "" &&
            formInput.openDate != "" &&
            formInput.closeDate != ""
        ...
        if (inputsValid) {
            const response = await createEvent(formInput)
                        ...
                        <Grid item xs={12} sm={12} md={12} container>
                            <Grid item xs={12} sm={12} md={6}>
                                <div className="form-group" style={{ display: 'flex', flexDirection: 'column', padding: '10px' }}>
                                    <label htmlFor="textInput" className="form-label">Event Name</label>
                                    <input
                            ...
                            <Grid item xs={12} sm={12} md={6}>
                                <div className="form-group" style={{ display: 'flex', flexDirection: 'column', padding: '10px' }}>
                                    <label htmlFor="textInput" className="form-label">Status</label>
                                    <input
                                        onChange={(event) => handleInput(event)}
                                        type={'text'}
                            ...
                            <Grid item xs={12} sm={12} md={6}>
                                <div className="form-group" style={{ display: 'flex', flexDirection: 'column', padding: '10px' }}>
                                    <label htmlFor="textInput" className="form-label">Event Details</label>
                                    <input
                                        onChange={(event) => handleInput(event)}
                                        type={'text'}
                                        value={formInput.eventDetails }
                                        style={{
                                            padding: '8px',
                                        }}
                                        className="form-control"
                            ...
                            <Grid item xs={12} sm={12} md={6}>
                                <div className="form-group" style={{ display: 'flex', flexDirection: 'column', padding: '10px' }}>
                                    <label htmlFor="textInput" className="form-label">Open Date</label>
                                    <input
                                        onChange={(event) => handleInput(event)}
                                        type={'date'}
                                        value={formInput.openDate}
                                        style={{
                                            padding: '8px',
                                        }}
                            ...
                            <Grid item xs={12} sm={12} md={6}>
                                <div className="form-group" style={{ display: 'flex', flexDirection: 'column', padding: '10px' }}>
                                    <label htmlFor="textInput" className="form-label">Close Date</label>
                                    <input
                                        onChange={(event) => handleInput(event)}
                                        type={'date'}
